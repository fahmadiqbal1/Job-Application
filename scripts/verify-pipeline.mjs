...
import { CANONICAL_STATES, normalizeStatus } from './lib/status-validator.mjs';
import { resolveAppsFile, resolveStatesFile, resolveReportsDir, resolveTrackerAdditionsDir, CAREER_OPS as CAREER_OPS_ROOT } from './lib/paths.mjs';

const APPS_FILE = resolveAppsFile();
const ADDITIONS_DIR = resolveTrackerAdditionsDir();
const REPORTS_DIR = resolveReportsDir();
const STATES_FILE = resolveStatesFile();

const CANONICAL_STATUSES = CANONICAL_STATES.map(s => s.toLowerCase());
...
for (const e of entries) {
  ...
  if (!CANONICAL_STATUSES.includes(statusOnly) && normalizeStatus(e.status).unknown) {
...
for (const e of entries) {
  ...
  if (!match) continue;
  const reportPath = join(CAREER_OPS_ROOT, match[1]);
