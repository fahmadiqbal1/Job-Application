...
export function CVEditor() {
  ...
  const { data } = useQuery({
    queryKey: ['cv'],
    queryFn: () => api.get<{ content: string }>('/settings/cv'),
  })
  const { mutate: save, isPending } = useMutation({
    mutationFn: (content: string) => api.put('/settings/cv', { content }),
  ...
  return (
    <GlassCard className="p-5">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-xs font-display font-semibold text-white/50 uppercase tracking-wider">CV (Markdown)</h3>
        <button
